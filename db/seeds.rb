...
# Define a method to create categories
def create_category(name)
  Category.find_or_create_by(category_name: name)
end

# Read data from CSV and create categories and products
CSV.foreach(Rails.root.join('/home/rmalhi/AWD/eCommerceProject/eCommerceProject/products.csv'), headers: true) do |row|
  category = create_category(row['category_name'])
  Product.create(
    product_name: row['product_name'],
    product_description: row['description'],
    product_price: row['price'].to_f,
    quantity_available: row['quantity'].to_i,
    category: category
  )
end
